 ...
 *  Transform SWF
 *
 *  Created by smackay on Fri Mar 21 2003.
...
namespace transform
{
    FSDefineText2::FSDefineText2(FSInputStream* aStream) :
        FSDefineObject(DefineText2, 0),
        bounds(),
        transform(),
        textRecords()
    {
    ...
    const char* FSDefineText2::className() const
    {
        const static char _name[] = "FSDefineText2";
    
        return _name;
    }

    void FSDefineText2::add(const FSVector<FSText>& anArray)
    {
        for (FSVector<FSText>::const_iterator i = anArray.begin(); i != anArray.end(); ++i)
            textRecords.push_back(*i);
    }

    int FSDefineText2::lengthInStream(FSOutputStream* aStream)
    {
    ...
    void FSDefineText2::encodeToStream(FSOutputStream* aStream)
    {
    ...
    void FSDefineText2::decodeFromStream(FSInputStream* aStream)
    {
        int glyphBits = 0;
        int advanceBits = 0;
        int location = 0;
    ...
    int FSDefineText2::glyphBits() const
    {
        int numberOfBits = 0;
    ...
    int FSDefineText2::advanceBits() const
    {
        int numberOfBits = 0;
